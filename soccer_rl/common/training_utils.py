...
Import from ``soccer_rl.common.training_utils`` or the repo-root shim ``training_utils``.
"""
...
try:
    from soccer_rl.common.rewards import RewardShapingWrapper
except Exception:
    RewardShapingWrapper = None

from soccer_rl.common.env_goal_tracking import EnvGoalTrackingWrapper
...
def create_rllib_env(env_config=None):
    ...
    if hasattr(context, "keys") and callable(getattr(context, "keys", None)):
        ...
        config_dict = {}
    # Optional: force Unity worker_id (e.g. render_match) — RLlib otherwise overwrites
    ...
    env = EnvGoalTrackingWrapper(env, goal_reward_threshold=goal_th)
    if (
