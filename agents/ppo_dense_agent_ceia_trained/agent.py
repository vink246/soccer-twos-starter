...
Network shape expected by this package: 336-dim obs, Discrete(27), mlp_actor_critic [512,512].
"""
...
_CHECKPOINT_ENV = "PPO_DENSE_AGENT_CEIA_TRAINED_CHECKPOINT"


class PPODenseAgentCeiaTrained(AgentInterface):
    """Loads a PPO .pth and acts for both team players with the same policy (watch)."""

    def __init__(self, env: gym.Env):
        super().__init__()
        self.name = "PPO dense CEIA trained (checkpoint)"
        self.env = env
        ...
        if os.path.isfile(weights_path):
            ...
            raise FileNotFoundError(
                ...
                "to agents/ppo_dense_agent_ceia_trained/checkpoint.pth or set %s."
    ...
    def act(self, observation: Dict[int, np.ndarray]) -> Dict[int, np.ndarray]:
        ...
        if self._use_flattener:
            return {
                keys[0]: self.flattener.lookup_action(a0),
                keys[1]: self.flattener.lookup_action(a1),
            }
